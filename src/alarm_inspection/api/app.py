...
from alarm_inspection.storage import Inspection, PointDecision, PointList, SourceFile, open_store
...
<input type="hidden" name="point_decisions" id="point-decisions" value="[]"><button type="submit">Create inspection</button></form><div id="message"></div></div><div id="preview-modal" class="modal"><div class="modal-card"><button class="close" id="close-preview">Close</button><h2>Points List Review</h2><p id="preview-summary"></p><p>Edit descriptions directly. Mark a row for deletion to remove it from the working output.</p><table><thead><tr><th>Point</th><th>Description</th><th>Status</th><th>Reason</th><th>Delete</th></tr></thead><tbody id="preview-body"></tbody></table><button type="button" id="save-review">Save review decisions</button></div></div>
...
</script><script>const originalPreview=document.querySelector('#preview-list');const decisionField=document.querySelector('#point-decisions');const saveReview=document.querySelector('#save-review');const reviewBody=document.querySelector('#preview-body');originalPreview.addEventListener('click',()=>setTimeout(()=>{for(const row of reviewBody.querySelectorAll('tr')){const cells=row.querySelectorAll('td');if(cells.length<5)continue;const description=cells[1].textContent;cells[1].innerHTML='<input class="review-description" value="'+description.replaceAll('"','&quot;')+'">';cells[4].innerHTML='<input class="review-delete" type="checkbox">'}},500));saveReview.addEventListener('click',()=>{const decisions=[...reviewBody.querySelectorAll('tr')].map(row=>{const cells=row.querySelectorAll('td');return {address:cells[0].textContent?Number(cells[0].textContent):null,text:cells[1].querySelector('input').value,accepted:cells[2].textContent==='Accepted',deleted:cells[4].querySelector('input').checked,reason:cells[3].textContent}});decisionField.value=JSON.stringify(decisions);document.querySelector('#preview-modal').classList.remove('open');document.querySelector('#message').textContent='Review decisions saved. Creating the inspection will commit them.'});</script></body></html>"""
...
def create_app():
    ...
    async def create_inspection(
        ...
        with store.begin() as session:
            ...
            for decision in json.loads(point_decisions):
                session.add(PointDecision(id=str(uuid4()), inspection_id=inspection_id,
                                          ...
                                          deleted=bool(decision.get("deleted", False)),
